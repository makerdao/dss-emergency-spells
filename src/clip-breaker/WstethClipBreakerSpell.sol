...
contract WstethClipBreakerSpell is DssEmergencySpell {
    ...
    string public constant description =
        string(abi.encodePacked("Emergency Spell | Set Clip Breaker: ", WSTETH_A, ", ", WSTETH_B));

    event SetBreaker(bytes32 indexed ilk, address indexed clip);

    function _emergencyActions() internal override {
        _setBreaker(WSTETH_A);
        _setBreaker(WSTETH_B);
    }
