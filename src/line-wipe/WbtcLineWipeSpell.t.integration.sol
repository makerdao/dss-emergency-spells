...
import {WbtcLineWipeSpell} from "./WbtcLineWipeSpell.sol";
...
contract WbtcLineWipeSpellTest is DssTest {
    ...
    function testAutoLineWipeOnSchedule() public {
        ...
        assertGt(pmaxLine, 0, "WBTC-A before: auto-line maxLine already wiped");
        assertGt(pgap, 0, "WBTC-A before: auto-line gap already wiped");
        assertFalse(spell.done(), "WBTC-A before: spell already done");

        (pmaxLine, pgap,,,) = autoLine.ilks(WBTC_B);
        assertGt(pmaxLine, 0, "WBTC-B before: auto-line maxLine already wiped");
        assertGt(pgap, 0, "WBTC-B before: auto-line gap already wiped");
        assertFalse(spell.done(), "WBTC-B before: spell already done");

        (pmaxLine, pgap,,,) = autoLine.ilks(WBTC_C);
        assertGt(pmaxLine, 0, "WBTC-C before: auto-line maxLine already wiped");
        assertGt(pgap, 0, "WBTC-C before: auto-line gap already wiped");
        ...
        assertEq(maxLine, 0, "WBTC-A after: auto-line maxLine not wiped");
        assertEq(gap, 0, "WBTC-A after: auto-line gap not wiped (gap)");
        assertTrue(spell.done(), "WBTC-A after: spell not done");

        (maxLine, gap,,,) = autoLine.ilks(WBTC_B);
        assertEq(maxLine, 0, "WBTC-B after: auto-line maxLine not wiped");
        assertEq(gap, 0, "WBTC-B after: auto-line gap not wiped (gap)");
        assertTrue(spell.done(), "WBTC-B after: spell not done");

        (maxLine, gap,,,) = autoLine.ilks(WBTC_C);
        assertEq(maxLine, 0, "WBTC-C after: auto-line maxLine not wiped");
        assertEq(gap, 0, "WBTC-C after: auto-line gap not wiped");
